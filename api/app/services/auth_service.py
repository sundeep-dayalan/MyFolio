...
class AuthService:
    ...
    async def process_google_oauth_callback(self, code: str, state: str) -> Tuple[UserResponse, Token]:
        ...
        try:
            ...
            google_user_info = await self.google_oauth.verify_and_get_user_info(id_token)
            
            # Try to get user from database, fallback to mock user if Firestore is disabled
            try:
                ...
                # If Firestore is disabled, create a mock user for testing
                if "SERVICE_DISABLED" in str(db_error) or "Cloud Firestore API" in str(db_error):
                    logger.warning("Firestore is disabled, creating mock user for testing")
                    user = UserResponse(
    ...
    async def authenticate_google_user(self, google_credential: str) -> Tuple[UserResponse, Token]:
