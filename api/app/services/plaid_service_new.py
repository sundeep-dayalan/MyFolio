...
from ..utils.token_security import encrypt_access_token, decrypt_access_token, mask_token_for_logging
from ..models.plaid import (
    PlaidAccessToken, PlaidTokenStatus, PlaidEnvironment,
    PlaidAccountWithBalance, PlaidBalance, PlaidAccount
)
...
class PlaidService:
    ...
    def _setup_plaid_client(self):
        ...
        try:
            ...
            logger.info(f"Plaid client initialized for environment: {settings.PLAID_HOST}")
            ...
            for key, value in obj.items():
                if not str(key).startswith('_') and not callable(value):
                    try:
                        ...
                        logger.debug(f"Skipping key {key} due to serialization error: {e}")
                        continue
            return cleaned_dict
    ...
    async def get_stored_access_token(self, user_id: str) -> Optional[PlaidAccessToken]:
        ...
        try:
            ...
            query = tokens_ref.where('user_id', '==', user_id).where('status', '==', PlaidTokenStatus.ACTIVE.value)
            ...
            latest_doc = max(docs, key=lambda d: d.get('created_at', datetime.min.replace(tzinfo=timezone.utc)))
            ...
            logger.info(f"Retrieved Plaid token for user {user_id}, item: {plaid_token.item_id}")
            return plaid_token
    ...
    async def store_access_token(self, user_id: str, access_token: str, item_id: str, 
                               ...
                               institution_name: Optional[str] = None) -> PlaidAccessToken:
        """
        ...
        try:
            ...
            logger.info(f"Stored encrypted Plaid token for user {user_id}, item: {item_id}")
    ...
    async def update_token_last_used(self, user_id: str, item_id: str):
        ...
        try:
            tokens_ref = self.db.collection(self.PLAID_TOKENS_COLLECTION)
            query = tokens_ref.where('user_id', '==', user_id).where('item_id', '==', item_id)
    ...
    async def create_link_token(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            request = LinkTokenCreateRequest(
                products=[Products('transactions'), Products('assets'), Products('accounts')],
                client_name="Personal Wealth Management",
                country_codes=[CountryCode('US')],
                language='en',
                user=LinkTokenCreateRequestUser(client_user_id=user_id),
                webhook=settings.PLAID_WEBHOOK_URL if hasattr(settings, 'PLAID_WEBHOOK_URL') else None
            )
            ...
            raise PlaidServiceError(f"Failed to create link token: {error_response.get('error_message', str(e))}")
        except Exception as e:
    ...
    async def exchange_public_token(self, user_id: str, public_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Successfully exchanged and stored token for user: {user_id}, item: {item_id}")
            ...
            raise PlaidServiceError(f"Token exchange failed: {error_response.get('error_message', str(e))}")
        except Exception as e:
    ...
    async def get_accounts(self, user_id: str) -> List[PlaidAccountWithBalance]:
        ...
        try:
            ...
            if not stored_token:
                raise PlaidServiceError("No active Plaid connection found. Please reconnect your bank account.")
            ...
            if error_response.get('error_code') == 'ITEM_LOGIN_REQUIRED':
                ...
                raise PlaidServiceError("Bank connection expired. Please reconnect your account.")
            
            raise PlaidServiceError(f"Failed to get accounts: {error_response.get('error_message', str(e))}")
