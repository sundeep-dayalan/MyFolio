...
def test_token_exchange():
    ...
    headers = {"Content-Type": "application/json", "X-Dev-User-ID": USER_ID}
    ...
    response = requests.post(
        f"{API_BASE}/plaid/exchange_public_token", headers=headers, json=exchange_data
    )
    ...
    if response.status_code == 200:
        ...
        if data.get("accounts"):
            ...
            for i, account in enumerate(data["accounts"][:3]):  # Show first 3
                ...
                print(
                    f"        Type: {account.get('type', 'Unknown')} - {account.get('subtype', 'Unknown')}"
                )
