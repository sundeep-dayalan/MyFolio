...
class JWTKeyService:
    ...
    def __init__(self):
        ...
        if self.key_vault_url:
            try:
                ...
                logger.info("Azure Key Vault JWT crypto client initialized successfully")
            except Exception as e:
                logger.error(f"Failed to initialize JWT Key Vault client: {e}")
                raise RuntimeError(f"Key Vault initialization failed: {e}")
        else:
            logger.error("Key Vault URL not configured. JWT operations require Key Vault.")
    ...
    def create_access_token(self, data: dict, expires_delta: Optional[timedelta] = None) -> str:
        ...
        if not self.crypto_client:
            raise RuntimeError("Key Vault not configured. JWT operations require Key Vault.")
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                if header.get('alg') != 'RS256':
                    logger.error(f"Invalid token algorithm: {header.get('alg')}. Only RS256 tokens are accepted.")
                    return None
                
                result = self._verify_token_with_keyvault(token)
                logger.debug(f"Token verification result: {'SUCCESS' if result else 'FAILED'}")
                return result
            else:
        ...
        try:
            ...
            if verification_result.is_valid:
                ...
                if payload.get('exp'):
                    ...
                    logger.debug(f"Token exp: {token_exp}, Current time: {current_time}, Valid: {current_time < token_exp}")
